class GuiBlock:
    ...
    def __init__(self,label=""):
        ...
        prevent_call = create_proxy(lambda e:e.stopPropagation())
        self.div.elt.addEventListener('mousedown', prevent_call)

    def __getattr__(self, attr):
        return self.data[attr]
    
    def setValue(self,attr,value):
        self.setters[attr](value)
    ...
    def _addElement (self,name,element,valueFun=lambda el:el.value(),display=True):
        ...
        #sp = "&nbsp;"*(10-len(name))
        self.elems[name+"label"]=label=createSpan(name.replace("_"," "))
