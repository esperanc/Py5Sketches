...
float sceneSDF(vec3 p) {
    ...
    float cyl = cylinderSDF(p-vec3(0,0,1),0.4,0.6);
    return min(min(ball,block),cyl);
}

vec4 sceneMat (vec3 p) {
    float ang = iTime*3.14/10.;
    float ball = sphereSDF(p-vec3(-1.,0,0),0.8);
    float block = blockSDF(opRotY(p-vec3(1.,0,0),ang), vec3(0.6));
    float cyl = cylinderSDF(p-vec3(0,0,1),0.4,0.6);
    float d = min(min(ball,block),cyl);
    if (d == ball) return vec4(1,1,0,1);
    if (d == cyl) return vec4(1,0,1,1);
    return vec4(0,1,1,0);
}
...
const float matSpec = 0.; // Specular coefficient
...
vec3 lighting (vec3 eye, inout vec3 p, out vec3 R) {
    ...
    vec4 matColor = sceneMat(p);
    vec3 ia = matAmb * lightColor * matColor.rgb;
    vec3 id = matDiff * max(0.,dot(l, n)) * lightColor * matColor.rgb;
    vec3 r = reflect(-l,n);
    ...
    if (matColor.a > 0.) {
        ...
        // Refracts
        vec3 Rin = refract(-e,n,1./refrIndex);
        // Ray march inside the solid
        vec3 pInside = p -n*EPSILON*5.; 
        float dOut = rayMarch(pInside, Rin, -1.);
        vec3 pOut = pInside+dOut*Rin;
        vec3 nOut = normal(pOut);
        R = refract(Rin, -nOut, refrIndex);
        if (length(R)==0.) {
            // Total reflection: try once more
            Rin = reflect(Rin, nOut);
            pInside = pInside - nOut*EPSILON*5.;
            dOut = rayMarch(pInside, Rin, -1.);
            pOut = pInside+dOut*Rin;
            nOut = normal(pOut);
            R = refract (Rin, -nOut, refrIndex);
            if (length (R)==0.) {
                // Another T.R. - forget it
                R = Rin;
            }
        }
        p = pOut + nOut * 5. * EPSILON;
    }
    return (ia+id+is);
}
